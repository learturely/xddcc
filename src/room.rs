...
impl Room {
    ...
    pub fn get_all_rooms<'a, Iter: Iterator<Item = &'a Session> + Clone>(
        ...
        Room::get_all_live_id(&sessions.clone().collect(), Arc::clone(&map));
        let rooms = Arc::new(Mutex::new(HashMap::new()));
        for session in sessions {
            Room::id_to_rooms(map.clone(), (*session).clone(), rooms.clone());
            break;
        }
        Arc::into_inner(rooms).unwrap().into_inner().unwrap()
    }
    pub fn get_all_live_id(sessions: &Vec<&Session>, id_map: Arc<Mutex<HashMap<String, i64>>>) {
